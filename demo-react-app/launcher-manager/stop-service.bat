...
:: TransactionSaga удалена
if "%SERVICE_NAME%"=="TransactionSaga" (
  echo ⚠️  TransactionSaga удалена из системы
  exit /b 0
)
...
:: Специальная обработка для фронтенда
if "%SERVICE_NAME%"=="KefirFrontend" (
  if "!PID!"=="FRONTEND_RUNNING" (
    echo 🛑 Останавливаю KefirFrontend по порту...
    call :kill_by_port 3000
    goto :cleanup
  )
  if "!PID!"=="PORT_CHECK_ONLY" (
    echo 🛑 Останавливаю по проверке порта...
    call :kill_by_port 3000
    goto :cleanup
  )
)
